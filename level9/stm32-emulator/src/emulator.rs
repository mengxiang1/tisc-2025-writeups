...
pub fn dump_stack(uc: &mut Unicorn<()>, count: usize) {
    ...
    info!("=== Register Dump ===");
    for &reg in &regs {
        if let Ok(val) = uc.reg_read(reg) {
            info!("{:?} = 0x{:08x}", reg, val);

            // Try dereferencing the register if it points to RAM/ROM
            let addr = val as u32;
            if (0x0800_0000..0x2002_0000).contains(&addr) {
                let mut buf = [0u8; 4];
                if uc.mem_read(addr.into(), &mut buf).is_ok() {
                    let v = u32::from_le_bytes(buf);
                    info!("    *0x{:08x} = 0x{:08x}", addr, v);
                }
            }
        }
    }
    ...
    info!("=== Stack Dump === (SP=0x{:08x})", sp);

    let mut sp_addr = sp;
    for _ in 0..count {
        let mut v = [0u8; 4];
        if uc.mem_read(sp_addr, &mut v).is_err() {
            info!("stack dump finished due to mem read error at 0x{:08x}", sp_addr);
            break;
        }
        let val = u32::from_le_bytes(v);

        if (0x0800_0000..0x0810_0000).contains(&val) {
            info!("*** 0x{:08x} (sp=0x{:08x})", val, sp_addr);
        } else {
            info!("    0x{:08x} (sp=0x{:08x})", val, sp_addr);
        }
        ...
        sys.uc.borrow_mut().add_intr_hook(move |uc, exception| {
            match exception {
                ...
                3 => {
                    ...
                    error!("Prefetch Abort / HardFault at pc={:08x}", pc);
