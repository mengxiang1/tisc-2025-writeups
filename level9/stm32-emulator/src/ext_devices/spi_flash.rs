...
impl SpiFlash {
    pub fn new(config: SpiFlashConfig) -> Result<Self> {
        ...
        debug!(
            "spi-flash: loaded {} bytes (file {}) size requested={:#x}",
            content.len(),
            &config.file,
            config.size
        );
...
impl ExtDevice<(), u8> for SpiFlash {
    ...
    fn read(&mut self, _sys: &System, _addr: ()) -> u8 {
        match self.reply.as_mut() {
            Some(Reply::Data(d)) => {
                let b = d.pop_front().unwrap_or_default();
                trace!("{} -> 0x{:02x} (from Data)", self.name, b);
                b
            }
    ...
    fn try_process_command(&mut self, cmd: Command, args: &[u8]) -> Option<Reply> {
        match (cmd, args) {
            ...
            (Command::ReadData, [a, b, c]) => {
                ...
                // Copy the flash content from addr onwards into a VecDeque and return that
                // as the immediate reply. This avoids subtle shared-state/borrowing problems
                // and better models "I have N bytes ready to send".
                let end = self.config.size;
                let slice = &self.content[addr..end];
                let dq: VecDeque<u8> = slice.to_vec().into();
                debug!(
                    "{} cmd={:?} ReadData addr=0x{:06x} len={}",
                    self.name,
                    cmd,
                    addr,
                    slice.len()
